...
class Databases extends ServiceProvider
{
    protected $db;          //the db connection

    protected $sql;         //sql statement

    public $status;         // database connection status
    ...
    {
        $config = require_once CONFIG_PATH . "providers/database.config.php";
        ...
        try {
            ...
            Logger::save('database', 'connecting to database : ' . $db_name , 'Success');
        ...
        } catch(PDOException $e)
        {
            ...
            Logger::save('database', 'connecting to database ' , 'Fail');
    ...
    public function __destruct()
    {
        ...
        Logger::save('database', 'closing to database ' , 'Success');
    }
    ...
    public function query($sql){
        ...
        catch( PDOException $e )
        {
            ...
            Logger::save('database', $sql , 'Fail');

            $stmt = $this->error()[0] .' : ' . $this->error()[1] . ' | Error SQL statement | ' . $this->sql . ' | ' . $this->error()[2] ;

            Logger::save('database', $stmt , 'ERROR' );

            die($stmt);

        }

        Logger::save('database', $sql , 'Success');
    ...
    public function getData($type=false, $from, $select="*", $where="", $group_by="", $having="", $order_by="", $ending="", $limit=null)
    {
        $group_by = ($group_by == null || !is_integer($group_by))? null : 'GROUP BY  ' . $group_by;

        $having = ($having == null || !is_integer($having))? null : 'HAVING  ' . $having;

        $order_by = ($order_by == null || !is_integer($order_by))? null : 'ORDER BY  ' . $order_by;

        $ending = ($ending == null || !is_integer($ending))? null :  $ending;

        $limit = ($limit == null || !is_integer($limit))? null : 'limit = ' . $limit;

        $query = "SELECT $select FROM $from $where $group_by $having $order_by $ending $limit";
